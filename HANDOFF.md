...
- **Phase 3.5 — expensive verbs + the native floor. ◐ inspect built (2026-06-14).** The scope turned out narrower than billed: **fonts / layer / constraints are already live** — the walker captures them and `node`/`tree`/`find --include` expose them over the socket. The one genuinely-new value-fetching verb, **`inspect`, is now built and verified by real injection**: `command.uitool.inspect` + `domain.uitool.registry` (the node-id → live-object resolver; v1 re-walks the live tree, the weak map is the deferred optimization) + `ObjectInspector` (reads ivar *values* — the part `RuntimeKit` deliberately omitted — via `object_getIvar`/offset memory reads, screened by `RuntimeSafety`) + the gated `--invoke` getter path over **`RuntimeKitC`** (the native safety floor's first piece — an ObjC `@try/@catch` shim). Verified: `inspect --invoke` a live `NSVisualEffectView` reads its `material` (7) and the target survives. The tractable verb batch is also **built (2026-06-14):** **`schema`** (static — prints the output contract + exit-code map, with a drift-guard test that caught an undocumented `truncated` field), **`signing`** (static — reads a target's code signature via the Security framework + derives the cooperative-injectability verdict; SampleAppKitApp→true, Calculator→false), and **`classes`** (injected — lists the target's loaded classes by `--match` or reflects one via `RuntimeMirror`, run **off-main** since a 30k-class enumeration blows the main-thread bound; the `objc_copyClassList`+`free` crash was found and replaced with `objc_getClassList`). **What remains of 3.5:** the two heavy verbs deliberately deferred — **`ax-diff`** (accessibility-vs-view-tree) and **`swiftui`** (private/unstable SwiftUI runtime) — plus the rest of `RuntimeKitC`'s native floor (pointer-validity probing, Swift-class isa decode, heap enumeration). Each is its own `/sdd-clarify` + scope call.
...
- **Distributed as a developer tool, never embedded in a product.** `uitool` and its `UIToolBoot` dylib install from source like any other RE/debug CLI — a Homebrew formula builds and signs them. The binaries stay gitignored build outputs, not committed blobs; what never happens is baking the injector into an app you ship. The arm64e unrestricted injectable stays dev-box only — the machine is defanged, the file is not secret.
