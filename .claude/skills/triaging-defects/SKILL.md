...
2. **Reproduce.** For non-trivial defects, invoke `systematic-debugging` and follow the four-phase discipline. For runtime/visual issues, reproduce against the `SampleAppKit` oracle or by running `flexscope` against a live target, and observe the behavior firsthand (see `macos-development` → Verifying).
...
- `macos-development` (→ Verifying) — reproducing runtime defects against the `SampleAppKit` oracle or a live target.
