...
# `id:` in Specs/ or Features/), a `Sources/` target, a harness area, a feature
# slug, a name listed in `.claude/commit-scopes`, or `treewide`. The set is
# derived from the filesystem at commit time — adding a spec, a `Sources/<target>`
# dir, or a `.claude/commit-scopes` line makes that scope usable with no list to
# edit. Handles `git commit -m "..."` and the HEREDOC `-m "$(cat <<'EOF'
...
#   - target scopes: the immediate subdirs of Sources/
...
allowed+=$'\n'$({ ls -d "$root"/Sources/*/ 2>/dev/null || true; } | sed -E 's#.*/Sources/##; s#/$##')
