...
- Detaching does not terminate the target. For a Path A relaunched process, who owns that process's lifecycle after detach is defined by the injection half (where Path A is built): in the cheap-read MVP, detach removes only the inspection bridge and never kills the target, so a relaunched process keeps running and its lifecycle is the researcher's concern. See [[story.uitool.attach-release]] scenario.uitool.attach-release.app-survives-relaunched.
