...
Create `Features/<tool>/<NNNN>-<slug>/` if it doesn't exist (next number, kebab-case slug, under the owning tool's namespace). Copy templates from `.Codex/templates/feature/`:
...
After the user approves the feature folder, commit the spec content. See `.Codex/rules/commit-discipline.md` for message style.
