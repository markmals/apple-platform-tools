...
- **Fresh context, different model.** Dispatch a new subagent that never saw the code get built. Default it to `model: "opus"` even though the earlier reviewers ran on `sonnet` — the cognitive diversity is the point. Tell it to read `.Codex/skills/adversarial-review/SKILL.md` and apply it.
...
Once both reviews pass and `/sdd-verify` is green, commit. See `.Codex/rules/commit-discipline.md` for message style.

Natural boundaries per spec applied (the spec ID is the commit scope — see `.Codex/rules/commit-discipline.md`):
