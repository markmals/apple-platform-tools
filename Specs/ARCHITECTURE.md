...
- **Distributed as a developer tool, never embedded in a product.** `uitool` and its `UIToolBoot` dylib install like any other reverse-engineering / debugging CLI (lldb, `class-dump`, Reveal, Hopper) — built from source and shipped as a Homebrew formula. The binaries stay build outputs (`.gitignore`d, produced on install), not committed blobs; what must never happen is the reverse — baking the injector or the injection step into an app you ship. The unrestricted **arm64e** injectable and the machine defang it needs stay dev-box only: not because the artifact is secret, but because the machine it runs on is deliberately weakened.
...
The reframe: the defanged machine is the **worst case, not the floor**. For your own apps, `uitool` is a stock-Mac tool. The injection half (`UIToolBoot` / `UIToolServer`) is built — the cooperative posture works on a stock Mac today (spawn-inject via `launch`; attach-to-running via the debugger entitlement). Only the unrestricted posture adds the arm64e injectable and the machine defang.
