...
**The cost of one package, named honestly:** a single `swift-tools-version` and one platform floor for the whole graph. We pin the floor at the common denominator and gate higher-OS features per-target with `@available`. Tools with hard *runtime* requirements (uitool needs Tahoe + arm64e + a defanged machine) enforce those at runtime via a `doctor` verb, not via the package manifest. arm64e build flags, codesigning, and injection are **build-script** concerns, never baked into a shippable product.
...
| **Live runtime introspection** | `uitool` | `RuntimeKit` (+ `UIToolBoot`) | "What is this *running* process actually doing?" — real view-tree, fonts, constraints, ivars, via injection + the ObjC runtime. |
...
**macOS today.** The tools are host-side CLIs that read binaries, SDKs, and other macOS processes. The package floor sits at the common denominator of the absorbed code; per-tool higher-OS needs are gated with `@available`, and uitool's Tahoe/arm64e/defanged-machine requirement is a **runtime** precondition (`uitool doctor`), not a manifest constraint.
...
- **The injection tool is defanged-machine-only.** `uitool` requires SIP + AMFI + library-validation off — a system-wide regression. That machine holds no real data or credentials. This is verified at runtime by `uitool doctor`, documented once, and never softened.
