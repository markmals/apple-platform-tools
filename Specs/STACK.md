...
| Signing | `codesign` ad-hoc; the injectable is built **arm64** for cooperative (your own apps) and **arm64e** (`--options runtime`, `disable-library-validation`) to match the system shared cache for unrestricted (apps you did not sign). The cooperative arm64 build ships as a source-built Homebrew formula (ad-hoc signed, not notarized); the arm64e unrestricted build stays dev-box only |
