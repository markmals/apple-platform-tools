...
struct CorpusIntegrityTests {
  ...
  @Test func `the corpus decodes and is non-empty`() throws {
    let corpus = try loadedCorpus()
    #expect(!corpus.patterns.isEmpty)
  }

  @Test func `the corpus contains the full Phase 1b taxonomy`() throws {
    let corpus = try loadedCorpus()
    #expect(corpus.patterns.count == 69)
  }

  @Test func `pattern ids are unique and kebab-case`() throws {
    let corpus = try loadedCorpus()
    var seen = Set<String>()
  ...
  @Test func `required text fields are non-empty`() throws {
    let corpus = try loadedCorpus()
    for p in corpus.patterns {
  ...
  @Test func `the HIG reference is present and plausible`() throws {
    let corpus = try loadedCorpus()
    for p in corpus.patterns {
      #expect(!p.higReference.section.isEmpty, "\(p.id): empty HIG section")
      #expect(
        p.higReference.url.hasPrefix(
          "https://developer.apple.com/design/human-interface-guidelines"),
        "\(p.id): HIG url not a HIG page: \(p.higReference.url)")
    }
  }

  @Test func `related ids resolve to existing patterns`() throws {
    let corpus = try loadedCorpus()
  ...
  @Test func `categories are within the known set`() throws {
    let corpus = try loadedCorpus()
    for p in corpus.patterns {
  ...
  @Test func `minMacOS parses as a version when present`() throws {
    let corpus = try loadedCorpus()
  ...
  @Test func `the shared instance matches a fresh load`() throws {
