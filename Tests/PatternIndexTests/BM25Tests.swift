...
struct BM25MathTests {
  @Test func `buildDoc accumulates weighted tf and length per token`() {
  ...
  @Test func `buildDoc sums tf and length across fields`() {
  ...
  @Test func `buildCorpus dedupes df per doc and computes avgDl`() {
  ...
  @Test func `buildCorpus counts each distinct term once per doc`() {
  ...
  @Test func `score matches the hand-computed single-term value`() {
  ...
  @Test func `score matches the hand-computed multi-term value`() {
    let a = BM25.buildDoc([("glass", 5.0)])
    let b = BM25.buildDoc([("table view", 3.0)])
    let corpus = BM25.buildCorpus([a, b])
  ...
  @Test func `the +1 idf guard keeps a common term positive`() {
  ...
  @Test func `score skips terms absent from the doc`() {
    let a = BM25.buildDoc([("glass", 5.0)])
  ...
  @Test func `countHits counts distinct present tokens`() {
...
struct RankingTests {
  ...
  @Test(.scenario("scenario.sdk-search.pattern-search.search"))
  func `ranks the concentric-corners pattern first for a concentric query`() throws {
    let results = try engine().search("concentric corners", max: 5)
    #expect(results.first?.id == "concentric-corner-configuration")
  }

  @Test(.scenario("scenario.sdk-search.pattern-search.search"))
  func `surfaces and ranks the glass-effect view first for a glass query`() throws {
  ...
  @Test(.scenario("scenario.sdk-search.pattern-search.search"))
  func `surfaces the split-view pattern for a sidebar query via the synonym map`() throws {
  ...
  @Test(.scenario("scenario.sdk-search.pattern-search.search"))
  func `ranks the table-view pattern first for a table query`() throws {
    let results = try engine().search("table view cell reuse", max: 5)
    #expect(results.first?.id == "tableview-view-based-reuse")
  }

  @Test func `the relevance floor drops weak runners-up`() throws {
  ...
  @Test func `the coverage gate uses raw tokens and requires half to match`() throws {
  ...
  @Test func `an all-stopword query returns no results`() throws {
